    ...
    for cycle_index, num_train_epochs in enumerate(schedule):
      ...
      if num_train_epochs:
        ...
        classifier.train(
            ...
            hooks=train_hooks,
            steps=train_steps_per_epoch*num_train_epochs if train_steps_per_epoch else None,
            max_steps=flags_obj.max_train_steps)
      ...
      eval_results = classifier.evaluate(input_fn=input_fn_eval,
                                         steps=eval_steps_per_epoch 
                                               if eval_steps_per_epoch else None)
                                         #steps=flags_obj.max_train_steps)
      if flags_obj.distribution_strategy == 'horovod':
        from mpi4py import MPI
        eval_avg_acc = MPI.COMM_WORLD.allreduce(eval_results['accuracy'],
                                                op=MPI.SUM)
